...
OmError_t om_decoder_init(OmDecoder_t* decoder, const OmVariable_t* variable, uint64_t dimension_count, const uint64_t* read_offset, const uint64_t* read_count, const uint64_t* cube_offset, const uint64_t* cube_dimensions, uint64_t lut_chunk_element_count, uint64_t io_size_merge, uint64_t io_size_max) {
    ...
    const uint64_t nLutChunks = divide_rounded_up(nChunks, lut_chunk_element_count);
...
bool om_decoder_next_data_read(const OmDecoder_t *decoder, OmDecoder_dataRead_t* data_read, const void* index_data, uint64_t index_data_size, OmError_t* error) {
    ...
    const uint64_t lutChunkElementCount = decoder->lut_chunk_element_count;
...
uint64_t _om_decoder_decode_chunk(const OmDecoder_t *decoder, uint64_t chunkIndex, const uint8_t* data, uint8_t* into, uint8_t* chunk_buffer) {
    uint64_t rollingMultiply = 1;
...
bool om_decoder_decode_chunks(const OmDecoder_t *decoder, OmRange_t chunk, const uint8_t* data, uint64_t data_size, uint8_t* into, uint8_t* chunkBuffer, OmError_t* error) {
    uint64_t pos = 0;
    ...
    for (uint64_t chunkNum = chunk.lowerBound; chunkNum < chunk.upperBound; ++chunkNum) {
        ...
        uint64_t uncompressedBytes = _om_decoder_decode_chunk(decoder, chunkNum, data + pos, into, chunkBuffer);
